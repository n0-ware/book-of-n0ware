...
Reading further down under [Usage](https://github.com/calebstewart/CVE-2021-1675#usage), we see this is ran by first calling the PowerShell [cmdlet](../../../tools_and_tricks/cli_utilities/powershell.md#command-lets)  `Invoke-Module` followed with a second command to create an admin user with a new password in the local admin group. The second command is the answer to the first question. 
